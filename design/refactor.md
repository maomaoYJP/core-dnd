...
问题：
当前hook的方法设置有问题，这些钩子都是在container中的，我需要分层设计钩子

1. session钩子，属于整个拖拽会话的生命周期，所有container共享
...
2. container钩子，属于某个container的生命周期，只有这个container会触发

具体钩子
onContainerEnter
onContainerLeave

两个钩子体系的区别是，session钩子是全局的，container钩子是局部的。session钩子可以在任何container中触发，container钩子只能在当前container中触发。

问题：
貌似不需要两个层级的hook，直接将这俩钩子移到session中，这样就用分两种钩子，功能也是完善的

现在是一开始就默认new了manager这样不太对好像
...
manager.mount(document.getElementById("listB"), {
  axis: "horizontal",
});
```

DragManager ← 应用层"工作台"，可有多个
↓ 持有
├─ hooks ← 插件注册中心
