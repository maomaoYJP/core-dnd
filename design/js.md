...
## 实现mouseUp后，元素回到insertIndex位置的动画效果

问题：
mouseUp后，渲染dom之后，利用FLIP动画，先将元素移动到幽灵元素的位置，然后过渡到目标位置。
但是这样有问题，因为容器的overflow是hidden，这时候如果使用上面方法，会被容器遮挡

改进思路：
直接将幽灵元素本身飞到目标位置，然后删除幽灵元素，这样就不会有被遮挡的问题了。
具体实现的时候，增加一个animateGhostToTarget方法，参数传入一个回调，回调是动画完成之后，需要执行的逻辑，比如删除幽灵元素，清理状态等。animateGhostToTarget方法内部会计算出目标位置，然后设置幽灵元素的transform进行动画，动画结束后调用回调函数。这样就能够实现mouseUp后，元素回到insertIndex位置的动画效果了。
...
onStart / onMove / onEnd / onAdd / onRemove

实现思路

1. 在container中，通过配置options传入回调
2. container加一个函数，用于触发事件

```javascript
triggerEvent(name, eventData) {
  this.options[name]?.(eventData)
}

// eventData 结构
eventData = {
  item,       // 被拖的 DOM 节点 = draggedItem.element（即 wrapper）
  from,       // 源容器 element
  to,         // 目标容器 element（同容器时 === from）
  oldIndex,   // 拖动开始时在源容器中的 index
  newIndex,   // 落点在目标容器中的 index（未发生移动的事件里可为 undefined）
  related,    // 仅 onMove：当前要插到它前面的那个 item.element，尾部追加时为 null
  willInsertAfter, // 仅 onMove：是否插在 related 之后
}
...
通过在可拖拽元素上设置特定的属性或类名，可以指定该元素的某个部分作为拖拽把手。这样，只有点击了这个把手，才能触发拖拽操作。

配置结构

```javascript
{
  handle: '.drag-handle',   // 字符串选择器；不传 = 整个 item（保持现状）
  filter: '.no-drag',       // 字符串选择器；命中则禁止拖动（比如按钮、输入框）
}

```

## 优化animateGhostToTarget体验

现在的流程

1. 松开鼠标 → session.end() 停掉 rAF 循环
2. 读 previewItem.element.getBoundingClientRect() —— 这就是问题所在
3. ghost 飞到读到的坐标
4. transitionend 后，takeNode/dropNode 把真实元素插入 DOM

问题：
快速拖动的时候，preview还在飞，ghost读到的是preview移动过程中的坐标，这时候回到原位的动画不正确。出现了闪烁和错位。

改进思路：
在定位飞行目标坐标时，不直接读 previewItem.element.getBoundingClientRect()，而是算出移动到的目标位置坐标，这样百分百正确

1. 通过 insertIndex 可以知道目标位置在容器内的视觉位置
2. 通过目标位置的视觉位置，可以算出目标坐标（相对于视口的坐标, start, crossStart）
3. 让 ghost 飞到这个目标坐标
